...
class AssemblStore
  ...
  constructor: (options) ->
    # If instantiated in legacy mode with annotator.addPlugin('AssemblStore', {...}),
    # then the first argument will be the Annotator's element. We don't need the
    # element: discard it.
    if arguments.length > 1
      options = arguments[1]

    @options = $.extend(true, {loadFromSearch:true}, @options, options)

    if @options.loadFromSearch
      Util.deprecationWarning("Use of the loadFromSearch option to the AssemblStore
                               plugin is deprecated. Please call
                               .load(queryObj) on the Annotator instance
                               instead.")

  # Support for legacy instantiation of AssemblStore plugin with .addPlugin('AssemblStore')
  pluginInit: ->
  ...
  _apiRequestOptions: (action, obj) ->
    ...
    if @options.annotationData?
      Util.deprecationWarning("Use of the annotationData option to the AssemblStore
...
Annotator.Plugin.register('AssemblStore', AssemblStore)

module.exports = AssemblStore
